...
const form = useForm({
  ...
  stock_quantity: props.reward.stock_quantity !== null && props.reward.stock_quantity !== undefined ? props.reward.stock_quantity : '',
  image: null,
  voucher_code_prefix: props.reward.voucher_code_prefix || '',
  validity_days: props.reward.validity_days || '',
  status: props.reward.status || 'active',
})

function submit() {
  // Ensure all required fields have values
  if (!form.category_id || !form.name || !form.points_required || !form.validity_days || !form.status) {
    alert('Please fill in all required fields')
    return
  }
  
  form.put(`/admin/rewards/${props.reward.id}`, {
    forceFormData: true,
    onSuccess: () => {
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
      <form @submit.prevent="submit" class="p-6 space-y-6">
        ...
        <div>
          ...
          />
          <p class="text-sm text-gray-500 mt-1">
            Upload a new image for the reward (optional)
          </p>
