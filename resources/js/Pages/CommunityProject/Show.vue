      ...
      <div class="px-6 py-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-3">
            <span :class="`px-3 py-1 text-sm font-medium ${statusColors[project.status]}`">
              {{ project.status }}
            </span>
    ...
    <div class="flex-1 max-w-7xl mx-auto p-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          <!-- Project Images -->
          <div v-if="project.photo_paths && project.photo_paths.length > 0" class="bg-white border border-gray-200 p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">Project Photos</h2>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div v-for="(photoPath, index) in project.photo_paths" :key="index" class="aspect-square">
                <img :src="`/storage/${photoPath}`" :alt="`Project photo ${index + 1}`" 
          ...
          <div class="bg-white border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-4">
              ...
              <div v-if="isAdmin" class="text-sm text-gray-500">Total updates: {{ project.news_history ? project.news_history.length : 0 }}</div>
            </div>
            
            <!-- News History with Scroll -->
            <div v-if="project.news_history && project.news_history.length > 0" class="max-h-96 overflow-y-auto space-y-4 pr-2">
              <div v-for="(news, index) in project.news_history" :key="news.id" class="bg-blue-50 border border-blue-200 p-4">
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center space-x-2">
                    <h3 class="text-sm font-medium text-blue-900">Update #{{ project.news_history.length - index }}</h3>
                    <span class="text-xs text-blue-600">by {{ news.creator_name }}</span>
                  </div>
                  <div class="flex items-center space-x-2">
                    <span class="text-xs text-blue-600">{{ formatDate(news.created_at) }}</span>
                ...
                <p class="text-gray-800 whitespace-pre-wrap">{{ news.content }}</p>
                
                <!-- Display files for this news entry -->
                <div v-if="news.files && news.files.length > 0" class="mt-3">
                  ...
                  <div class="grid grid-cols-1 gap-3">
                    <div v-for="(file, fileIndex) in news.files" :key="fileIndex" class="flex items-center justify-between bg-white border border-gray-200 p-3 rounded">
                      <div class="flex items-center space-x-3 min-w-0 flex-1">
                        <div class="w-8 h-8 bg-gray-100 flex items-center justify-center rounded flex-shrink-0">
                          <svg v-if="file.mime_type && file.mime_type.startsWith('image/')" class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <a :href="`/storage/${file.path}`" target="_blank" class="text-blue-600 hover:text-blue-800 text-sm font-medium flex-shrink-0 ml-3">
                        View
                      </a>
                    </div>
                  </div>
              ...
              <form @submit.prevent="updateLatestNews" class="space-y-4">
                ...
                <div>
                  ...
                  <div v-if="newsForm.news_files.length > 0" class="mt-3">
                    ...
                    <div class="space-y-2">
                      <div v-for="(file, index) in newsForm.news_files" :key="index" class="flex items-center justify-between bg-gray-50 p-2 rounded">
                ...
                <div class="flex justify-end space-x-3">
                  <button
                    ...
                    class="px-4 py-2 border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors"
                  >
                    Reset
                  </button>
                  <button
                    ...
                    class="bg-blue-600 text-white px-4 py-2 font-medium hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors disabled:opacity-50"
                  >
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white border border-gray-200 p-6">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <p class="text-gray-900">{{ project.volunteers_joined }}/{{ project.volunteers_needed }}</p>
          ...
          <!-- Admin Controls -->
          <div v-if="isAdmin" class="bg-white border border-gray-200 p-6">
          ...
          <div v-if="userApplication && !isAdmin" class="bg-white border border-gray-200 p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">Your Application</h2>
            <div class="space-y-3">
              <div>
                <label class="block text-sm font-medium text-gray-600">Status</label>
                <span :class="`px-2 py-1 text-xs font-medium ${statusColors[userApplication.status]}`">
                  {{ userApplication.status }}
                </span>
              </div>
              
              <div v-if="userApplication.status === 'approved'">
                <p class="text-green-600 font-medium">You're approved for this project!</p>
              ...
              <div v-if="userApplication.status === 'rejected' && userApplication.rejection_reason">
                ...
                <p class="text-sm text-gray-700 bg-red-50 p-3 border border-red-200">
    ...
    <div class="bg-white border-t border-gray-200 p-6">
      <div class="max-w-7xl mx-auto">
        <Link 
          ...
          class="w-full bg-gray-100 text-gray-700 py-3 px-4 font-medium hover:bg-gray-200 transition-colors text-center block"
        >
          ← Back to Projects
        </Link>
      </div>
